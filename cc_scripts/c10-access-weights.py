...
Hypothesis
==========
The learning rates for new neurons will be greater than their peers 
"""

# imports
import neurodl.cnn as cnn
from neurodl.adaptive_neurogenesis import *
import numpy as np
import pandas as pd
...
parser.add_argument(
    "-e", "--epochs", help="number of epochs to train", type=int, default=5
)
parser.add_argument(
...
results = pd.DataFrame(
    ...
    columns=["Average Learning Rate", "Group", "Repeat"],
)
...
for i in range(REPEATS):
    ...
    for group in group_config:
        ...
        log, adam_metadata = train_model_adaptive(
            model=net_copy,
            dataset=data_loader,
            dtype=dtype,
            device=device,
        ...
        avg_step_size = adam_metadata["step_sizes"][14]

        results.iloc[counter] = [avg_step_size, group, i]

        counter += 1


results.to_csv(EXP_RESULTS / "c10-access-weights-{}.csv".format(START_TIME))
print("Average Accuracy")
print(results.groupby("Group")["Average Learning Rate"].mean())
print("Done")
