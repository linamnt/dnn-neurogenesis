#!/bin/bash
#SBATCH --job-name=mlp-test-1000
#SBATCH --time=0-01:00            # days-hh:mm
#SBATCH --cpus-per-task=16        # Cores per GPU: Cedar - 6, Graham - 16
#SBATCH --gres=gpu:t4:1              #Request GPU "generic resources"
#SBATCH --mem=64000M
#
#SBATCH --output=./output/%x.out     # name-jobid-arrayidx.out
...
SCRIPT=mnist-test_1000.py # script should take as an argument the data path
