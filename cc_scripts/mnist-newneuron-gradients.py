...
def train(net, optimizer, criterion, parameters):
    ...
    """
    mlp.train_model(model=net, opt=optimizer, criterion=criterion, **parameters)
    return
...
def train_evaluation(parameters):
    ...
    for i in range(nfolds):
        net = mlp.NgnMlp(data_loader, layer_size=args.neurons, hidden=3)
        net.to(DEVICE)

        optimizer = optim.SGD(
            net.parameters(), lr=parameters["lr"], momentum=parameters["momentum"]
        )
