...
def load_data(
    mode, data_folder="./data", num_workers=16, batch_size=50, split=0.1, seed=23, fashion=False,
):
    """
    ...
    if fashion:
        trainset = torchvision.datasets.MNIST(
                    data_folder, train=True, transform=transforms.Compose(
                                        [transforms.ToTensor(),
    ...
    if mode == "validation":
        ...
        trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=True, 
        )
        print("Created data loaders")
    ...
    def __init__(
        ...
        if mode == "validation":
            ...
            self.train, self.test = load_data(
                mode=mode, data_folder=data_folder, seed=seed,
                fashion=fashion, batch_size=batch_size, num_workers=num_workers
            )
            self.valid = None
...
def ablation(model, dataset, step=0.05):
    ...
    for prop in proportions:
        ...
        acc = predict(model, dataset, train=False)
